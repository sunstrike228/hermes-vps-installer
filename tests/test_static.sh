...
grep -Fq 'v1.1.1' install.sh || fail "installer asset tag is not pinned"
...
grep -Fq 'npm_config_yes=true' install.sh || fail "npx auto-confirm is missing"
grep -Fq 'model.base_url https://chatgpt.com/backend-api/codex' install.sh || fail "codex base_url alignment is missing"
